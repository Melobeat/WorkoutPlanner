...
fun WorkoutSummaryScreen(
    ...
    val totalVolumeKg = uiState.exercises.sumOf { exercise ->
        exercise.sets.filter { it.isDone }.sumOf { set ->
            (set.weight.toDoubleOrNull() ?: 0.0) * set.effectiveReps()
        }
    }
                        ...
                        exercise.sets.forEachIndexed { index, set ->
                            ...
                            Row(
                                ...
                                Row(
                                    ...
                                    if (set.sideType == "Unilateral" && !isSkipped) {
                                        Surface(
                                            shape = CircleShape,
                                            color = MaterialTheme.colorScheme.primaryContainer
                                        ) {
                                            Text(
                                                stringResource(R.string.label_unilateral),
                                                style = MaterialTheme.typography.labelSmall,
                                                fontWeight = FontWeight.Bold,
                                ...
                                if (isSkipped) {
                                    ...
                                    val repsDisplay = set.displayReps()
                                    Text(
                                        stringResource(R.string.summary_reps_weight, repsDisplay, set.weight),
                                        style = MaterialTheme.typography.bodySmall,
                                        fontWeight = FontWeight.Medium
                                    )
                                }
